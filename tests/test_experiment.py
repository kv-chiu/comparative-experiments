import numpy as np
from comparative_experiments.experiment import SingleExperiment, ExperimentComparator  # Adjust the import path as necessary
...
def test_single_experiment_init():
    ...
    assert experiment.run_callable(np.array([1, 2, 3]), np.array([1, 2, 3])).all(), "The experiment run callable did not execute correctly."
...
def test_single_experiment_run():
    ...
    assert np.array_equal(experiment.run(X_test, y_test), X_test * 2), "The experiment run did not produce expected output."
...
def test_experiment_comparator_run():
    ...
    assert results["Mock Experiment"]["Dummy Metric"] == 1.0, "The experiment comparator did not compute metrics correctly."
